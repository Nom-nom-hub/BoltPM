...
//         println!("Contents of target/debug:");
//         if let Ok(entries) = std::fs::read_dir("../../target/debug") {
//             for entry in entries.flatten() {
//                 println!("  {:?}", entry.path());
//             }
//         }
//         println!("Contents of target/debug/deps:");
//         if let Ok(entries) = std::fs::read_dir("../../target/debug/deps") {
//             for entry in entries.flatten() {
//                 println!("  {:?}", entry.path());
//             }
//         }
//         panic!("Could not find libsample_plugin.dylib in any known location. Current working directory: {}. Tried: {:?}", std::env::current_dir().unwrap(), candidates);
//     }
//     let lib_path = candidates.iter().find(|p| p.exists()).unwrap();
